...
import { DbSchemaRequest, DbSchemaResponse, NestedTable, Settings, Table, SQLToMongoTemplate, TemplateSettings, Column, RequestStatus } from '../app.model';
import { FormField } from '@nikxsh/ngmodelform';
import { FormControl, Validators } from '@angular/forms';

@Component({
  selector: 'app-sqltomongo',
  templateUrl: './sqltomongo.component.html',
  styleUrls: [ './sqltomongo.component.css']
})
export class SqltomongoComponent implements OnInit {
  sqlFormFields: FormField[] = [];
  mongoFormFields: FormField[] = [];
  targetColumns: string[] = []
  dbSchema: DbSchemaResponse[] = []
  sampleDocument: any
  template: SQLToMongoTemplate = new SQLToMongoTemplate();
  ...
  onMainTableSelect(table: string, index: number) {
    ...
    if (mainTable)
      this.targetColumns = mainTable!.columns.map(x => x.name);
  ...
  onTableKeySelect(mainKey: string) {
    this.template.mainTable.key = mainKey;
    this.updateTempate();
  }
  ...
  public migrateData() {
    ...
    this.MigrationServiceRef.migrateSqlToMongo(JSON.parse(this.displayTemplate))
      .subscribe(response => {
